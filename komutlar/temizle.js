...
exports.run = async(client, message, args, ayar, emoji) => {
  ...
  if(!args[0] || (args[0] && isNaN(args[0])) || Number(args[0]) < 1 || Number(args[0]) > 400) return message.channel.send(embed.setDescription("1-400 arasında silinecek mesaj miktarı belirtmelisin!")).then(x => x.delete({timeout: 5000}));
