...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.permissions.has('ADMINISTRATOR')) return message.channel.send(nn.setImage('https://tenor.com/view/kin-of-the-stained-blade-yone-spin-the-sword-absorbed-the-azakana-league-of-legends-gif-17951773').setTitle(`Bir hata oldu!`).setThumbnail(message.author.avatarURL({dynamic: true})).setDescription(`**•** \`${client.ayarlar.prefix}küfür kısıt / kapat\` **kullanmak için,** \`Yönetici\` **yetkisine sahip olman gerekiyor.**`)).then(a => a.delete({timeout: 10000}));
